<script lang="ts">
	...
	export let options: string[] = [],
		noDefault = false,
		classes = '',
		selected: string | undefined = options[0],
		disabled = false,
		onSelect: (string) => void = null,
...
<div class="relative">
	<button
		class="{classes} {disabled
			...
			: 'cursor-pointer'} h-10 border rounded py-1 px-2 items-center flex justify-between space-x-4 whitespace-nowrap"
		on:click={() => {
			if (disabled) {
				return;
			}
