			...
			<div class="flex flex-col">
				<Button
					...
					onClick={() => {
						...
						notices = notices;
						selectedIndex = notices.length - 1;
					}}>새 공지사항</Button
				>
			</div>
			<div
				...
				<div class="flex flex-col">
					<div class="flex flex-col">
						...
						<input
							...
							type="text"
							bind:value={notices[selectedIndex]
								.title}
		...
		<div class="flex justify-end w-full mt-6">
			...
			<Button
				...
				onClick={async () => {
					...
					if (!notices[selectedIndex].contents) {
						return alert(
							`${notices[selectedIndex].title}의 내용을 입력해주세요.`
						);
					}
					//}
					if (confirm('저장하시겠습니까?')) {
						if (
							await createOrUpdateNotice(
								notices[selectedIndex]
							)
						) {
							alert('저장되었습니다.');
