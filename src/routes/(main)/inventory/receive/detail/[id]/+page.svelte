<script>
	...
	import { additionalReceive } from '../../../../../../stores/inventory';
			...
			<table class="w-full">
				...
				<tbody>
					{#each details as detail}
						<Tr>
							...
							<Td>
								<InputNumber
									placeholder="0"
									disabled={receive.state ===
										'종결'}
									classes="w-16 text-right"
									bind:value={detail.quantityAdditionalReceived}
									onInput={() => {
										if (
											detail.quantityAdditionalReceived
										) {
											detail.priceAdditionalReceived =
												detail.quantityAdditionalReceived *
												detail.priceVendor;
										} else {
											detail.priceAdditionalReceived = 0;
										}
									}}
								/>
							</Td>
					...
					<Tr>
						...
						<Td />
						<Td>
							{receive.quantityTotalAdditionalReceived}
						</Td>
						<Td classes="text-lg">
							{formatMoney(
								receive.priceTotalAdditionalReceived
							)}
						</Td>
					</Tr>
				</tbody>
		...
		{#if receive.state === '부분종결'}
			...
			<Button
				...
				onClick={async () => {
					...
					let input = [];
					details.forEach((d) => {
						if (d.quantityAdditionalReceived) {
							input.push({
								id: d.detailId,
								quantityAdditionalReceived:
									d.quantityAdditionalReceived
							});
						}
					});

					if (await additionalReceive(input)) {
						alert('종결되었습니다.');
