...
export default class CustomerService {
	static async getCustomers(variables?: { keyword?: string }) {
	...
	static async withdrawSmarterMoney(variables: {
		userId: number;
		amount: number;
		bank: string;
		...
		const client = HttpClient.getInstance();
		const result = (await client.request(CustomerMutation.CsWithdrawSmarterMoney, variables)) as {
			csWithdrawSmarterMoney: {
				success: boolean;
			};
		};
		return result.csWithdrawSmarterMoney.success;
	}
