...
export default class CsRequestQuery {
	...
	static CsRequestDetail = gql`
		query CsRequestDetail(
			...
			$getExtraOrderMaster: Boolean = false
			$orderMasterId: Int
			...
			csRequest(id: $id) {
				...
				gym {
					...
					orderMasters(page: $orderMasterPage) @include(if: $getOrderMasters) {
						orderMasters {
							id
							orderNumber
							dateCreated
			...
			orderMaster(orderMasterId: $orderMasterId) @include(if: $getExtraOrderMaster) {
				orderState
				id
				orderNumber
				dateCreated
